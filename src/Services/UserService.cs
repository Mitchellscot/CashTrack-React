...
{
    Task<User.Response> GetUserByIdAsync(int id);
    Task<User.Response[]> GetAllUsersAsync();
}
public class UserService : IUserService
{
    ...
    public async Task<User.Response[]> GetAllUsersAsync()
    {
        var users = await _userRepo.Find(x => true);
        return _mapper.Map<User.Response[]>(users);
    }

    public async Task<User.Response> GetUserByIdAsync(int id)
    {
